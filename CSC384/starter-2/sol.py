...
def heur_alternate(state):
    ...
    for box in state.boxes:
        if box not in state.storage:
            ...
            # check if at board corner but not at storage - not solveable
            if box in corners:
                return float("inf")

            # check if stuck in a corner inside the board
            ...
            for s in state.storage:
                ...
                if dist < min_d:
                    min_d = dist
            #box at wall but no storage space
