        ...
        if box not in state.storage:
            ...
            for s in state.storage:
                ...
                if s[1] == state.height - 1:
                    stor_at_uWall = True
                dist = abs(box[0] - s[0]) + abs(
                    box[1] - s[1])  # manhattan distance
                if dist < min_d:
                    min_d = dist
            ...
            if box[0] == 0 and (up in state.boxes or down in state.boxes):
                return float("inf")
            if box[0] == state.width - 1 and (
                    up in state.boxes or down in state.boxes):
                return float("inf")
            if box[1] == state.height - 1 and (
                    left in state.boxes or right in state.boxes):
                return float("inf")
